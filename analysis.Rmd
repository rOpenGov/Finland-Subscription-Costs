...
fees on scientific publishing in 2010-2017. The average annual costs for in Finland were `r round(mean((df %>% group_by(Year) %>% summarise(Costs = sum(Cost)/1e6))$Costs))` MEUR in 2010-2017.
...
geom_point() +
       ...
       scale_y_continuous(breaks = seq(1, max(dfs$Costs, na.rm = TRUE), 2))
print(p)
```
...
geom_point() +
       ...
       ylab("Relative cost increase") +
       scale_y_continuous(breaks = seq(1, max(dfs$Costs, na.rm = TRUE), .25))
print(p)
```
